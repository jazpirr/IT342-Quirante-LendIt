package edu.cit.quirante.lendit.feature.admin;

import edu.cit.quirante.lendit.feature.auth.User;
import edu.cit.quirante.lendit.feature.auth.UserRepository;
import edu.cit.quirante.lendit.feature.borrow.BorrowRequestRepository;
import edu.cit.quirante.lendit.feature.item.ItemImageRepository;
import edu.cit.quirante.lendit.feature.item.ItemRepository;
import edu.cit.quirante.lendit.feature.report.ReportRepository;
import edu.cit.quirante.lendit.feature.report.ReportService;
...
public class AdminController {
    ...
    public ResponseEntity<?> deleteItem(
        ...
        Integer userId = extractUserId(auth);
        if (!isAdmin(userId)) return ResponseEntity.status(403).body("Admin only");
        if (!itemRepo.existsById(itemId)) return ResponseEntity.status(404).body("Item not found");

        imageRepo.findByItemId(itemId).forEach(img -> imageRepo.delete(img));
        itemRepo.deleteById(itemId);
        if (reportId != null) reportService.updateStatus(reportId, "RESOLVED");
